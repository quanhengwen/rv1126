...
//新的环境变量校正需要的参数
EnvParam_t org_env_param = { 0,0,0,0 };             //计算完成后需要保存
EnvParam_t new_env_param = { 0,0,0,0 };             //计算完成后需要保存
uint8_t gain_flag = 1;

NucFactor_t nuc_factor = { 0,0,0 };                  //计算完成后需要保存
EnvFactor_t org_env_factor = { 0 };                 //计算完成后需要保存
EnvFactor_t new_env_factor = { 0 };                 //计算完成后需要保存
uint16_t nuc_table[NUCT_LEN] = { 0 };               //计算完成后需要保存
uint16_t correct_table[4 * 14 * 64];				//环境变量修正表
...
void point_temp_demo(uint16_t* temp_data, TempDataRes_t temp_res)
{
	...
	if (get_point_temp(temp_data, temp_res, point, &temp) == IRTEMP_SUCCESS)
	{
		...
		tempData = temp_value_converter(temp);
	}
}
